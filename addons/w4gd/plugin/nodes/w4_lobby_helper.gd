...
## Emitted while in an active WebRTC lobby when a new WebRTC mesh has been
## created.
## [br]
...
signal lobby_webrtc_mesh_created(webrtc_peer: WebRTCMultiplayerPeer)
...
## [signal lobby_webrtc_mesh_created] signal.
...
func _set_lobby(new_lobby: Lobby):
	...
	if lobby != null:
		...
		lobby.webrtc_mesh_created.disconnect(_lobby_webrtc_mesh_created)
	...
	if lobby:
		...
		lobby.webrtc_mesh_created.connect(_lobby_webrtc_mesh_created)
...
func _lobby_webrtc_mesh_created(peer: WebRTCMultiplayerPeer):
	if webrtc_autostart:
		multiplayer.multiplayer_peer = peer
	lobby_webrtc_mesh_created.emit(peer)
